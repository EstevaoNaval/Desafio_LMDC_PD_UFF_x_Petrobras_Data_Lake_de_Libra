...
request.onload = function () {
    ...
    if(request.status >= 200 && request.status < 400) {
        characters.forEach(character => {
            ...
            outputElementoGeral.insertAdjacentHTML('beforeend', `<a href='#listaPessoa${character.char_id}' aria-controls="listaPessoa${character.char_id}" data-toggle='list' role='tab' class='nav-link list-group-item list-group-item-action' id='outputLinhaNome${character.char_id}'"><h6>${character.name}</h6></a>`);
